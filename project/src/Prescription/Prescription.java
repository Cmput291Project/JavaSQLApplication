	...
	public Prescription(MedicalDB mDB, Console cs) {
		...
		while(flag) {
			...
			if(Input(cs) == 1) {
				...
				System.out.println("Record succesfully added.Thank you for using the prescription module");
				System.out.println("======================================");
				flag = false;
			}
		}
	...
	private boolean AddNew(MedicalDB mDB, Console cs) {
		...
		try {
			...
			String query = "INSERT INTO TEST_RECORD VALUES(" + 
					...
					"NULL, NULL, TO_DATE('"+presc_date + "', 'yyyymmdd'), NULL)";
		...
		try {
			while(correct) {
				...
				if(type == 3) {
					...
					ResultSet rs = mDB.connector.executeQuery("SELECT type_id, test_name FROM test_type WHERE test_name " +
							" LIKE '%"+ type_name +"%'");
					if(!rs.next()) {
