...
/*
 * Class to search for patients who have more than
 * alarming age for particular test type and have not
 * taken the test
 * 
 * CreateView() function references Assignment 2 solution to
 * Query number 8 by Thorey and a modification to Query number 9
 */
public class SearchByTestName {
	...
	private boolean ListItems(MedicalDB mDB, String test_name) {
		...
		ResultSet rs = mDB.connector.executeQuery("SELECT DISTINCT p.health_care_no, p.name, p.address, p.phone " +
							"FROM   patient p, medical_risk m, test_type tt " +
							"WHERE  trunc(months_between(sysdate,birth_day)/12) >= m.alarming_age " +
							"AND tt.test_name like '%" + test_name + "%' AND tt.type_id=m.medical_type " + 
							"AND p.health_care_no NOT IN (SELECT patient_no " +
                                "FROM   test_record t " +
                                "WHERE  m.medical_type=t.type_id AND t.result is not NULL)" +
